...
- **Clienti — colonna + filtri Lingua, e lingua nel modale**: aggiunta la colonna **Lingua** nella lista (bandiera + codice, sortabile), i **filtri per lingua** in coda alla riga dei filtri (chip inline accanto a Opted-out, con conteggio, toggle indipendente, mostrati solo se ≥2 lingue) e il **selettore lingua nel modale di modifica cliente**. La lingua viene ora salvata sulla tabella `clients` (colonna `lang`, migrazione `supabase/clients_lang.sql`): la lingua si deriva SOLO dalle prenotazioni **web/google** (scelta reale del cliente nel widget); walk-in/telefono hanno `lang='fr'` di default e vengono ignorati (era la falsa attribuzione «tutti FR»). Il valore impostato a mano nel modale (`clients.lang`) **prevale**; scegliere «—» lo azzera e torna all'auto-derivazione. Letture rese tolleranti se le colonne `clients.lang` / `reservations.source,lang` non ci sono (niente azzeramenti; foto/blocco preservati). Backend: `normLangCli()` valida contro le 10 lingue widget; GET arricchisce ogni cliente con `lang`; PATCH salva `lang` (con fallback se la colonna non è ancora migrata).
