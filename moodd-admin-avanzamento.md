...
- **Clienti — colonna + filtri Lingua, e lingua nel modale**: aggiunta la colonna **Lingua** nella lista (bandiera + codice, sortabile), i **filtri per lingua** in coda alla riga dei filtri (chip inline accanto a Opted-out, con conteggio, toggle indipendente, mostrati solo se ≥2 lingue) e il **selettore lingua nel modale di modifica cliente**. La lingua viene ora salvata sulla tabella `clients` (colonna `lang`, migrazione `supabase/clients_lang.sql`): la lingua è SOLO manuale (dal modale): NON si deriva più dalle prenotazioni, perché `reservations.lang` ha default 'fr' obbligatorio e attribuiva 'fr' a quasi tutti (falsa attribuzione + instabilità). Lettura clienti resa robusta se `clients.lang` non è ancora migrata (mantiene foto/blocco). Backend: `normLangCli()` valida contro le 10 lingue widget; GET arricchisce ogni cliente con `lang`; PATCH salva `lang` (con fallback se la colonna non è ancora migrata).
