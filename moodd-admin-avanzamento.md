...
- **Clienti — colonna + filtri Lingua, e lingua nel modale**: aggiunta la colonna **Lingua** nella lista (bandiera + codice, sortabile), i **filtri per lingua** sopra la lista (chip dinamici con conteggio, mostrati solo se ≥2 lingue presenti) e il **selettore lingua nel modale di modifica cliente**. La lingua viene ora salvata sulla tabella `clients` (colonna `lang`, migrazione `supabase/clients_lang.sql`): il dato curato dal modale prevale, altrimenti si ricava dalla prenotazione più recente. Backend: `normLangCli()` valida contro le 10 lingue widget; GET arricchisce ogni cliente con `lang`; PATCH salva `lang` (con fallback se la colonna non è ancora migrata).
